...
---
> In a closed network (where you know every device), NFS is a fine choice. <!--more--> With a good network, throughput it disgustingly fast and at the same time less CPU intensive on the server. It's very simple to set up and you can toggle readonly on shares you don't need to be writeable. - [NFS vs Samba](https://askubuntu.com/questions/7117/which-to-use-nfs-or-samba#7124)
...
This can be easily fixed: see [symlink]({% post_url 2021-10-18-symlink %}#auto-mounting-an-nfs-share)
...
### [Trouble shooting](https://chatgpt.com/share/697f5aca-67e8-800d-89f2-bcf5884a94e5)

see [ChatGPT](https://chatgpt.com/share/697f5aca-67e8-800d-89f2-bcf5884a94e5)
...
For advanced example of using autofs
see [Sharing home Folder]({% post_url 2025-11-24-network-share-home-folder %}#folder)
...
see [managing user 👤]({% post_url 2026-07-02-linux-managing-user %})


## [NFSv4 ⮺](https://chatgpt.com/share/6981055c-4cc4-800d-8b2a-947bbac74527)
  
[NFSv4](https://chatgpt.com/share/6981921f-3fec-800d-961c-a0e2cec542d5) is generally more secure and easy to secure than previous version.
{% highlight bash %}
$ sudo cat /proc/fs/nfsd/versions # show version supported by server
-2 -3 +4 +4.1 +4.2

$ rpcinfo -p # should show only v4 NFS services
{% endhighlight %}
  
Disabling other version at kernel level
{% highlight bash %}
# /etc/nfs.conf
[nfsd]
vers2 = n
...
And test again as shown above.

## [Bind NFS services](https://chatgpt.com/share/69809950-1668-800d-8de7-d34898658ac5)
  
- NFS does not bind to interfaces by name (like eth0), only to IP addresses. 
- NFSv4 uses only TCP 2049 and does not rely on rpcbind.

{% highlight ini %}
# /etc/nfs.conf for NFSv4
[nfsd]
host=192.168.10.5
...
Even if NFS binds correctly, you should still [restrict access to specific client](https://chatgpt.com/share/69809ef8-d7d8-800d-84a7-cb08063455dd):
...
- [x] This ensures only clients on that network can mount.
  
### Use firewall rules
- [x] Using [UFW]({% post_url 2026-02-02-network-firewall-ufw %}) (default on Linux Mint)
