---
title: Markdown
published: true
tags: markdown vscode jekyll doc
---

### [VSCode](https://code.visualstudio.com/Docs/languages/markdown)

- [display a Markdown syntax preview pane?](https://stackoverflow.com/questions/34125174/can-vscode-display-a-markdown-syntax-preview-pane) / [r/vscode](https://www.reddit.com/r/vscode/comments/1sn3rsm/opening_markdown_files_in_preview_mode_as_default/) - `ctrl+k v`
    - [Markdown All in One](https://marketplace.visualstudio.com/items?itemName=yzhang.markdown-all-in-one)
    - [Markdown Preview Enhanced](https://marketplace.visualstudio.com/items?itemName=shd101wyy.markdown-preview-enhanced)
    - [Markdown Editor Optimized](https://marketplace.visualstudio.com/items?itemName=vadimmelnicuk.meo) - interactive semi/preview
    
Images
- [Adding images and links to files to Markdown in VS Code](https://bobbyhadz.com/blog/vscode-add-image-to-markdown) - press shift & drag from image file

Runner
- [Markdown Command Runner](https://marketplace.visualstudio.com/items?itemName=Sycl.markdown-command-runner) - When clicked the associated snippet is run the current active terminal or a new one if it is busy.
- [Run your README.md like a notebook in VS Code ](https://dev.to/sourishkrout/run-your-readmemd-in-vs-code-50l7)

### Alternatives

- [Typst]({% post_url 2025-11-22-doc-typst %})
- [reStructuredText (RST, ReST, or reST)](https://en.wikipedia.org/wiki/ReStructuredText) - a file format for textual data used primarily in the Python programming language community for technical documentation. 

### see also
- [The Ultimate Guide to Writing a Great README.md for Your Project](https://medium.com/@kc_clintone/the-ultimate-guide-to-writing-a-great-readme-md-for-your-project-3d49c2023357)
- [	Markdown is holding you back](https://news.ycombinator.com/item?id=46017782) - args that markdown is missing _semantic markup_ and advise to use some alternative for better support for LLM.
	- [reStructuredText](https://www.sphinx-doc.org/en/master/usage/restructuredtext/index.html?utm_source=bphogan&utm_medium=email&utm_campaign=issue-45-markdown-is-holding-you-back)
    - [AsciiDoc](https://asciidoc.org/?utm_source=bphogan&utm_medium=email&utm_campaign=issue-45-markdown-is-holding-you-back)
