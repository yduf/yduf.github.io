...
It provide a very powerfull mechanism, that also allows to patch non-meson project to make them meson compatible, for eg see _implot_ setup in [GLFW + Skia + ImGui](https://github.com/yduf/glfw-skia-imgui/tree/master)

You will need
{% highlight bash %}
/subprojects/
/subprojects/my_package.wrap
/subprojects/packagefiles/my_package/your_patch  # which can be a meson file to build my_package
{% endhighlight %}

**Troubleshooting** 
- everything must be set right before /subprojects/my_package.wrap is extracted
- otherwise the patch is not applied (it is only applied once way extracting the wrap)

So if you need to adjust,
**don't forget to delete** `/supbroject/my_package` (the place where the wrap is extracted).

## Using [pkg-config ⮺](https://stackoverflow.com/a/68932575/51386)

Meson provides the _dependency()_ abstraction for working with pkg-config, cmake, various *-config tools, and a few built in dependencies that Meson has to save everyone reimplementing them, you would simply write:
