...
| ★★★☆☆ | longcat-2.0  | 2026-09-01 | 📝 | ▶ | Context:&nbsp;1.0M _free_ with a daily&nbsp;cap | used as a substitute to deepseek |
...
Restricting agent access - [using a dedicated user](https://chatgpt.com/share/6a9a9979-b6b4-83ed-bfee-5b26bcedf401)

```bash
$ sudo groupadd agent  # dedicated group for agent
$ sudo useradd --system --create-home --shell /usr/sbin/nologin --gid agent agent  # dedicatd user

$ sudo usermod -aG agent "$USER"        # add yourself to the agent group (so you can share files & folder)

```

**Don't**
- give agent `sudo` access
- make agent a member of broad groups such as docker, libvirt, adm, this effectively provide root-level access.
</div>

On linux you also can leverate ACL.

Then 
```bash
$ sudo -u agent agent-cli
