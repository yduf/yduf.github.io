...
Parport 0 may be sufficient (first port detected on host), otherwise:
{% highlight bash %}
$ dmesg | grep parport
...
- [#4: PyVCP](https://www.youtube.com/watch?v=H41WDH25nMQ) / gladeVCP - **V**irtual **C**ontrol **P**anel that extend `axis`  ui (button / slider / readout) 
...
## LinuxCNC/Debian
- [Avahi is an mDNS/DNS daemon, service which implements Apples’s Zeroconf mechanism.](https://www.poftut.com/linux-avahi-daemon-tutorial-examples/) `$ systemctl stop avahi-daemon`